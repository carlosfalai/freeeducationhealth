 ...
 * with mock credentials. (Same pattern and env var names as bot/config.cjs,
 * kept self-contained so history-insights/ works as a standalone module.)
 *
 ...
 *   REPORT_LOCALE    Fallback BCP-47 locale for the report (default "en").
 *
 * jurisdiction is intentionally never set here -- this is a person-facing
 * module, so per core/INTERFACE.md it omits jurisdiction, leaving
 * billing-code suggestions to physician-facing front-ends only.
 */
...
function parseProviders(raw) {
  if (!raw || !raw.trim()) {
    throw new Error(
      'PANEL_PROVIDERS is not set. Configure at least two AI providers, e.g.\n' +
        ...
        'See history-insights/.env.example.'
    );
  }
  ...
  return raw.split(',').map((entry) => {
    ...
    // shred that URL, so re-join everything past the third part.
  ...
  if (providers.length < panelSize) {
    throw new Error(
      `PANEL_PROVIDERS configures ${providers.length} provider(s) but PANEL_SIZE requires ${panelSize}. ` +
        'Add more providers to PANEL_PROVIDERS or lower PANEL_SIZE (never below 2).'
    );
  }
